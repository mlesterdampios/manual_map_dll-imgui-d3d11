...
/*static void ShaderDump(LPCWSTR ext, const void* data, DWORD size)
{
    HANDLE f = CreateFileW(ext, GENERIC_WRITE, 0, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ImGui_ImplDX11_Data* bd = ImGui_ImplDX11_GetBackendData();
    if (!bd->pd3dDevice) {
        const char* msg = "!bd->pd3dDevice";
        //ShaderDump(L"pd3dDevice", msg, sizeof(msg));
        return false;
    }
    ...
    // Create the vertex shader
    {
        ...
        if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, vertexShaderBlob, sizeof(vertexShaderBlob), &bd->pInputLayout) != S_OK)
        {
            return false;
        }
